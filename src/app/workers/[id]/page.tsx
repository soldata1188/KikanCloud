...
export default async function WorkerDetailPage(props: { params: Promise<{ id: string }> }) {
    ...
    return (
        <div className="flex h-screen bg-[#f0f4f9] font-sans text-[#1f1f1f] overflow-hidden selection:bg-blue-100">
            ...
            <main className="flex-1 flex flex-col relative overflow-y-auto no-scrollbar print:bg-white lg:print:ml-64 print:ml-0">
                ...
                <header className="flex justify-between items-center px-4 py-3 md:px-6 md:py-4 sticky top-0 bg-[#f0f4f9] z-10 print:hidden">
                ...
                <div className="flex-1 flex flex-col px-4 pb-12 w-full max-w-[900px] mx-auto mt-4 md:mt-6 print:m-0 print:p-0 print:w-full print:max-w-none">
                    ...
                    <div className="hidden print:block text-center mb-6 pt-8">
                        ...
                        <div className="text-right text-xs text-gray-500">出力日: {new Date().toLocaleDateString('ja-JP')}</div>
                    </div>
 ...
 <div className="w-32 h-32 rounded-3xl bg-gray-50 border-2 border-gray-100 overflow-hidden flex items-center justify-center shrink-0 print:border-gray-300 print:rounded-[32px]">
 ...
 <span className="text-sm font-semibold text-blue-600 bg-blue-50 px-3 py-1 rounded-[32px] border border-blue-100 print:border-gray-500 print:text-black print:bg-transparent">
                                ...
                                <p className="text-lg text-gray-500 font-medium tracking-widest mb-5">{worker.full_name_kana || 'カナ未設定'}</p>

                                <div className="flex flex-wrap gap-4 text-sm font-medium text-gray-600 print:text-black">
                                    <span className="flex items-center gap-2 bg-gray-50 px-3 py-1.5 rounded-lg border border-gray-100 print:border-gray-300 print:bg-transparent"><Globe2 size={16} className="text-gray-400 print:text-black" /> {worker.nationality || '未設定'}</span>
                                    <span className="flex items-center gap-2 bg-gray-50 px-3 py-1.5 rounded-lg border border-gray-100 print:border-gray-300 print:bg-transparent"><Briefcase size={16} className="text-gray-400 print:text-black" /> {sysTypeLabels[worker.system_type as keyof typeof sysTypeLabels] || worker.system_type}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <DetailItem label="受入企業" value={worker.companies?.name_jp || <span className="text-gray-400 italic">未割当</span>} />
