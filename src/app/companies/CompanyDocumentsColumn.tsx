...
import { FileText, UploadCloud, Trash2, Eye, FilePlus, FolderOpen, X, Check } from 'lucide-react';
import { saveCompanyDocumentRecord, deleteCompanyDocumentRecord } from './company-documents-actions';
...
const DOC_TYPES: Record<string, { label: string; color: string; bg: string }> = {
    ...
    company_register: { label: '登記簿', color: 'text-emerald-700', bg: 'bg-emerald-100' },
    financial: { label: '決算書', color: 'text-blue-700', bg: 'bg-blue-100' },
    license: { label: '許可証', color: 'text-amber-700', bg: 'bg-amber-100' },
...
interface CompanyDocumentsColumnProps {
    companyId: string | null;
}

export default function CompanyDocumentsColumn({ companyId }: CompanyDocumentsColumnProps) {
    const [docs, setDocs] = useState<CompanyDoc[]>([]);
    const [loading, setLoading] = useState(false);
    const [uploading, setUploading] = useState(false);
    const [uploadDocType, setUploadDocType] = useState(DOC_TYPES['contract'].label);
    ...
    const confirmUpload = async () => {
        ...
        try {
            ...
            const path = `${companyId}/${Date.now()}_${formattedFileName}`;
            ...
            await saveCompanyDocumentRecord({
                companyId,
                docType: docTypeKey,
    ...
    const handleDragEnter = (e: React.DragEvent) => {
        e.preventDefault();
        e.stopPropagation();
        if (companyId && !uploading && !pendingFile) setIsDragging(true);
    };
    ...
    const handleDragOver = (e: React.DragEvent) => {
        e.preventDefault();
        e.stopPropagation();
        if (companyId && !uploading && !pendingFile && !isDragging) setIsDragging(true);
    };
    ...
    const handleDrop = (e: React.DragEvent) => {
        ...
        e.stopPropagation();
        setIsDragging(false);
        if (!companyId || uploading || pendingFile) return;
        const file = e.dataTransfer.files?.[0];
        if (file) handleFileSelect(file);
    };


    const handleView = async (doc: CompanyDoc) => {
        try {
    ...
    const handleDelete = async (doc: CompanyDoc) => {
        ...
        try {
            const { error } = await supabase.storage.from('company_docs').remove([doc.file_path]);
            if (error) {
                console.warn('Storage file could not be removed:', error.message);
            }
            await deleteCompanyDocumentRecord(doc.id);
            await fetchDocs();
        } catch (err: any) {
    ...
    if (!companyId) {
        return (
            <div className="h-full flex flex-col items-center justify-center text-center p-6 bg-white">
                ...
                <p className="text-[13px] font-black text-gray-300">企業を選択してください</p>
                <p className="text-[10px] text-gray-200 mt-1 leading-relaxed">
                    書類を管理するには<br />左のリストから企業を選択します
                </p>
            </div>
        );
    }
            ...
            <div className="px-4 py-3 bg-white border-b border-gray-100 shrink-0">
                <div className="max-w-[400px] mx-auto w-full space-y-2">
                    {!pendingFile ? (
                        ...
                        <div className="flex flex-col gap-2 bg-blue-50/50 p-2.5 rounded-lg border border-blue-100 shadow-sm">
                            ...
                            <div className="flex flex-col gap-2 mt-1">
                                <input
                                    ...
                                    disabled={uploading}
                                    placeholder="種類 (例: 契約書)"
                                    className="w-full h-8 px-2 bg-white border border-blue-200 rounded-md text-[11px] font-bold outline-none focus:border-blue-500 transition-all text-blue-800 shadow-sm"
                                />
                                ...
                                <div className="flex gap-1.5 shrink-0 justify-end">
                                    <button
                                        ...
                                        disabled={uploading}
                                        className="flex-1 h-8 px-2 bg-white hover:bg-slate-50 border border-blue-200 text-slate-500 rounded-md text-[11px] font-bold transition-all shadow-sm"
                                    >
                                        取消
                                    </button>
                                    <button
                                        ...
                                        disabled={uploading}
                                        className="flex-1 h-8 px-2 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white rounded-md text-[11px] font-black flex items-center justify-center gap-1.5 transition-all shadow-sm shadow-blue-600/20"
                                    >
                                        {uploading
