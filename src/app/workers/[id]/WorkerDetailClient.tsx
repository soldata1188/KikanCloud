...
const DetailPulse = ({ date, label, color = "bg-[#0067b8]" }: { date: string, label: string, color?: string }) => {
    ...
    return (
        <div className="flex flex-col items-center absolute -top-1" style={{ left: `${percent}%` }}>
            <div className={`w-2.5 h-2.5 rounded-full border-2 border-white hover:scale-125 transition-transform ${color}`} />
            <span className="text-[8px] font-black text-gray-400 absolute top-4 whitespace-nowrap uppercase tracking-tighter">{label}</span>
        </div>
    );
};
