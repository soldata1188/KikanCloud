...
export function Sidebar({ active }: { active: 'dashboard' | 'workers' | 'companies' }) {
    return (
        <aside className="w-[68px] flex flex-col items-center py-4 shrink-0 bg-[#f0f4f9] z-20">
            ...
            <button className={`w-12 h-12 flex items-center justify-center rounded-full mb-2 transition-colors ${active === 'companies' ? 'bg-[#c2e7ff] text-[#001d35]' : 'hover:bg-black/5 text-[#444746]'}`} title="受入企業（準備中）">
