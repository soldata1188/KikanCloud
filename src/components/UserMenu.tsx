...
export function UserMenu({ displayName, email, role, avatarUrl }: { displayName: string, email: string, role?: string, avatarUrl?: string | null }) {
 ...
 className="flex items-center gap-2 pl-4 pr-1.5 py-1.5 bg-white rounded-md text-sm font-medium text-[#1f1f1f] hover:bg-gray-50 transition border border-gray-350 cursor-pointer"
 >
 {displayName}
 {avatarUrl ? (
 ...
 {isOpen && (
 <div className="absolute right-0 mt-2 w-72 bg-white rounded-[24px] border border-gray-350 overflow-hidden z-50 animate-in fade-in zoom-in-95 duration-200">
 <div className="p-4 border-b border-[#e1e5ea] bg-gradient-to-br from-[#f0f4f9] to-white">
 <div className="flex items-center gap-3">
 {avatarUrl ? (
 <img src={avatarUrl} alt={displayName} className="w-12 h-12 rounded-md object-cover shrink-0"/>
 ) : (
 <div className="w-12 h-12 rounded-md bg-[#d81b60] text-white flex items-center justify-center text-xl font-bold shrink-0">
 ...
 : 'bg-primary-100 text-primary-700 border border-green-200'
 }`}>
