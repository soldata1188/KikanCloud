  ...
  return (
    <div className="flex h-screen bg-[#fbfcfd] font-sans text-[#1f1f1f] overflow-hidden selection:bg-[#24b47e]/20">
      ...
      <div className="flex-1 flex flex-col relative min-w-0">
        <TopNav title="ダッシュボード" role={userProfile?.role} />
                  ...
                  <tbody className="divide-y divide-[#ededed]">
                    {workers?.map(w => (
                      <tr key={w.id} className="hover:bg-[#fbfcfd] transition-colors group">
                        ...
                        <td className="px-5 py-4"><span className={`px-2 py-0.5 border rounded-full text-[10px] font-bold uppercase tracking-widest bg-transparent ${w.status === 'working' ? 'border-[#24b47e] text-[#24b47e]' : 'border-gray-300 text-[#878787]'}`}>{w.status === 'working' ? '就業中' : w.status === 'returned' ? '帰国' : w.status}</span></td>
