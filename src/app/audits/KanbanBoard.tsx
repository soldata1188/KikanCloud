...
export function KanbanBoard({ audits }: { audits: any[] }) {
 ...
 <span className="bg-white/60 text-[#444746] text-xs font-bold px-2 py-1 rounded-[32px]">{audits.filter(a => a.status === col.id).length}</span> 
                    </div>
                    ...
                    <div className="space-y-4 flex-1">
                        {audits.filter(a => a.status === col.id).map(audit => {
                            ...
                            return (
                                <div key={audit.id} className={`bg-white p-5 rounded-[24px]  hover:shadow-md transition-all group relative`}>
