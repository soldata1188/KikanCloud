...
export function BrandModal({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) {
 ...
 <div
 ref={modalRef}
 className={`relative bg-white w-full max-w-[800px] rounded-[24px] 0_20px_40px_rgba(0,0,0,0.2)] overflow-hidden flex flex-col md:flex-row transition-all duration-400 ease-[cubic-bezier(0.34,1.56,0.64,1)] transform ${isOpen ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-95 translate-y-4'}`}
 >
 <button onClick={onClose} className="absolute top-4 right-4 z-50 w-8 h-8 flex items-center justify-center rounded-md bg-white/80 border border-gray-350 text-[#878787] hover:text-[#1f1f1f] hover:bg-gray-50 transition-colors md:hidden">
 <X size={16} strokeWidth={2.5} />
 </button>

 {/* 左ブロック: 折り紙クラウド */}
 <div className="md:w-5/12 p-8 bg-white border-b md:border-b-0 md:border-r border-gray-350 flex flex-col items-center justify-center relative group min-h-[250px] md:min-h-[350px]">
 ...
 <style>{`
 .origami-modal { filter: (0 15px 25px rgba(36, 180, 126, 0.25)); transition: transform 0.6s cubic-bezier(0.34, 1.56, 0.64, 1); }
...
`}</style>
 ...
 <button onClick={onClose} className="absolute top-4 right-4 z-50 w-8 h-8 hidden md:flex items-center justify-center rounded-md bg-white border border-gray-350 text-[#878787] hover:text-[#1f1f1f] hover:bg-gray-50 hover:border-gray-350 transition-all">
 <X size={16} strokeWidth={2.5} />
 </button>

 <span className="px-3 py-1 bg-[#24b47e]/10 text-[#24b47e] text-[9px] font-bold uppercase tracking-[0.2em] rounded-md border border-[#24b47e]/20 w-fit mb-4">Brand Identity</span>
 ...
 <div>
 <div className="flex items-center gap-2 mb-1.5"><div className="w-1.5 h-1.5 bg-[#24b47e] rounded-md"></div><h3 className="text-[11px] font-bold text-[#878787] uppercase tracking-wider">01. Concept & Shape</h3></div>
 ...
 <div>
 <div className="flex items-center gap-2 mb-1.5"><div className="w-1.5 h-1.5 bg-[#24b47e] rounded-md"></div><h3 className="text-[11px] font-bold text-[#878787] uppercase tracking-wider">02. Our Mission</h3></div>
 <p className="text-[12px] text-[#666666] leading-relaxed pl-3.5">書類業務を完全にデジタル化・自動化し、担当者が本来すべき「実習生や受入企業への支援」に注力できる世界を創り出します。</p>
 </div>
 </div>

 <div className="mt-8 pt-5 border-t border-gray-350">
