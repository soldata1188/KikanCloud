            ...
            <div className="flex items-center gap-3 w-full sm:w-auto justify-end">
                {layout && onLayoutChange ? (
                    <div className="flex items-center bg-white border border-gray-200 rounded-md p-0.5">
                        <button onClick={() => onLayoutChange('grid')} className={`p-1 rounded ${layout === 'grid' ? 'bg-white text-[#1f1f1f] shadow-sm border border-gray-200' : 'text-[#878787] hover:text-[#1f1f1f] hover:bg-[#ededed]'}`}><Grid size={14} /></button>
                        <button onClick={() => onLayoutChange('list')} className={`p-1 rounded ${layout === 'list' ? 'bg-white text-[#1f1f1f] shadow-sm border border-gray-200' : 'text-[#878787] hover:text-[#1f1f1f] hover:bg-[#ededed]'}`}><List size={14} /></button>
                    </div>
                ) : (
                    <div className="flex items-center bg-white border border-gray-200 rounded-md p-0.5">
                ...
                {role === 'admin' && (
                    <button onClick={handleExport} className="group relative h-[32px] w-[32px] flex items-center justify-center bg-white border border-gray-200 hover:bg-gray-50 text-[#1f1f1f] rounded-md transition-colors shadow-sm shrink-0">
                ...
                {addLink && (role === 'admin' || role === 'staff') && (
                    <Link href={addLink} className="group relative h-[32px] w-[32px] flex items-center justify-center bg-[#24b47e] hover:bg-[#1e9a6a] text-white rounded-md transition-colors shadow-sm shrink-0">
                        <Plus size={16} />
                        <div className="absolute top-full mt-2 px-2 py-1 bg-[#1f1f1f] text-white text-[11px] font-medium rounded-sm opacity-0 invisible translate-y-[-5px] group-hover:opacity-100 group-hover:visible group-hover:translate-y-0 transition-all duration-200 whitespace-nowrap z-[60] pointer-events-none">
                            新規登録
                        </div>
