            ...
            <main className="flex-1 flex flex-col relative overflow-y-auto no-scrollbar">
                <div className="flex-1 flex flex-col px-4 pb-16 w-full max-w-[900px] mx-auto mt-4 md:mt-8">
                    ...
                    <div className="flex flex-wrap items-center gap-2 mb-8 p-1.5 bg-white border border-gray-200 rounded-[20px] w-fit shadow-sm">
                        ...
                        <Link href={`/workers/${id}/documents`} className="flex items-center gap-2 px-6 py-2.5 rounded-[14px] text-sm font-bold transition-colors text-gray-500 hover:text-gray-800 hover:bg-gray-50"><FolderOpen size={18} /> 関連書類</Link>
                        <Link href={`/workers/${id}/timeline`} className="flex items-center gap-2 px-6 py-2.5 rounded-[14px] text-sm font-bold transition-colors bg-blue-50 text-[#4285F4] shadow-sm pointer-events-none"><Map size={18} /> 手続ロードマップ</Link>
                    </div>

                    {!worker.entry_date && (
                        <div className="mb-6 p-4 bg-yellow-50 border border-yellow-200 rounded-2xl flex items-start gap-3 text-yellow-800 text-sm">
                    ...
                    <div className="bg-white rounded-[32px] shadow-sm border border-[#e1e5ea] p-8 md:p-12">
                        <div className="flex items-center justify-between mb-10 pb-6 border-b border-gray-100">
                            <div>
                                <h3 className="text-2xl font-bold text-[#1f1f1f] flex items-center gap-3"><Map className="text-[#4285F4]" size={28} /> 法定ロードマップ (Timeline)</h3>
                                <p className="text-sm text-gray-500 mt-2">
                                    制度区分: <span className="font-bold text-[#4285F4] bg-blue-50 px-2 py-0.5 rounded">{getSystemTypeName(worker.system_type)}</span><br />
