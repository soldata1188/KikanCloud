...
export function SmartDrawer({ workerId, documents, title, docType, icon, allowUpload = true }: { workerId: string, documents: any[], title: string, docType: string, icon: React.ReactNode, allowUpload?: boolean }) {
    ...
    return (
        <div className="bg-white rounded-[24px] shadow-sm border border-teal-100 overflow-hidden mb-4">
            <button onClick={() => setIsOpen(!isOpen)} className="w-full px-6 py-4 flex items-center justify-between bg-white hover:bg-teal-50/50 transition-colors">
                <div className="flex items-center gap-3">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${allowUpload ? 'bg-orange-50 text-orange-600' : 'bg-teal-50 text-teal-600'}`}>{icon}</div>
            ...
            {isOpen && (
                <div className="px-6 pb-6 pt-2 border-t border-gray-50 bg-gray-50/50">
                    {allowUpload && (
                        <div className="mb-6 flex gap-4 items-end bg-white p-4 rounded-2xl border border-gray-200">
                            <div className="w-1/3">
                                <label className="block text-xs font-bold text-gray-500 mb-1">対象月</label>
                                <input type="month" value={targetMonth} onChange={(e) => setTargetMonth(e.target.value)} disabled={isPending} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 text-sm outline-none focus:border-orange-400" />
                            </div>
                            <div className="flex-1 relative">
                                ...
                                <button disabled={isPending} className="w-full py-2 bg-orange-50 hover:bg-orange-100 text-orange-600 font-bold text-sm rounded-xl border border-orange-200 transition-colors flex items-center justify-center gap-2">
                    ...
                    <div className="space-y-2 max-h-[300px] overflow-y-auto pr-2">
                        {filteredDocs.length === 0 ? <p className="text-sm text-gray-400 text-center py-4">データがありません</p> : filteredDocs.map(doc => (
                            <div key={doc.id} className="flex items-center justify-between p-3 bg-white border border-gray-100 rounded-xl">
                                ...
                                {doc.signedUrl && <a href={doc.signedUrl} target="_blank" rel="noopener noreferrer" className="p-2 bg-gray-50 hover:bg-teal-50 text-gray-600 hover:text-teal-600 rounded-lg transition-colors shrink-0"><Download size={16} /></a>}
                            </div>
                        ))}
                    </div>
                </div>
