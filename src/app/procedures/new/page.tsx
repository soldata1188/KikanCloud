            ...
            <main className="flex-1 flex flex-col relative overflow-y-auto no-scrollbar">
                <div className="flex-1 flex flex-col px-4 pb-12 w-full max-w-[800px] mx-auto mt-4 md:mt-8">
                    <div className="flex items-center gap-4 mb-8 pl-2">
                        <Link href="/procedures" className="w-10 h-10 flex items-center justify-center rounded-full hover:bg-black/5 transition-colors text-[#444746]"><ArrowLeft size={24} strokeWidth={1.5} /></Link>
                    ...
                    <form action={createProcedure} className="space-y-6">
                        <div className="bg-white rounded-[32px] shadow-sm border border-[#e1e5ea] p-8">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div className="md:col-span-2 pt-4 border-t border-gray-100"><label className="block text-sm font-bold text-[#444746] mb-2">対象人材</label><select name="worker_id" className="w-full bg-[#f0f4f9] focus:bg-white border border-transparent focus:border-[#4285F4] rounded-2xl px-4 py-3 outline-none appearance-none cursor-pointer text-[#1f1f1f] transition-all"><option value="">個人を指定しない（企業単体の手続）</option>{workers?.map(w => <option key={w.id} value={w.id}>{w.full_name_romaji} ({(w.companies as any)?.name_jp || '未配属'})</option>)}</select></div>
                                ...
                                <div><label className="block text-sm font-bold text-[#444746] mb-2">初期ステータス</label><select name="status" defaultValue="preparing" className="w-full bg-[#f0f4f9] focus:bg-white border border-transparent focus:border-[#4285F4] rounded-2xl px-4 py-3 outline-none appearance-none cursor-pointer text-[#1f1f1f] transition-all"><option value="preparing">書類準備中 (未提出)</option><option value="submitted">申請済・審査待ち</option><option value="completed">完了 (許可/認定/合格)</option></select></div>
                        ...
                        <div className="pt-4 pb-8 flex justify-end gap-3 sticky bottom-4"><Link href="/procedures" className="px-6 py-3 text-[#444746] bg-white border border-[#e1e5ea] font-bold hover:bg-gray-50 rounded-full transition-colors shadow-sm">キャンセル</Link><SaveButton /></div>
                    </form>
                </div>
            </main>
        </div>
