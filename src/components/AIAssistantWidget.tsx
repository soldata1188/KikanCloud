...
const TypingIndicator = () => (
 ...
 <div className="bg-gray-100 border border-gray-350 px-4 py-3 rounded-md rounded-tl-sm flex items-center space-x-1.5 w-fit h-[38px]">
...
export function AIAssistantWidget() {
 ...
 <div
 className={`
 mb-4 w-full sm:w-[360px] bg-white border border-gray-350 rounded-md flex flex-col transition-all duration-300 origin-bottom-right
 ${isOpen ? 'opacity-100 scale-100 translate-y-0 h-[550px]' : 'opacity-0 scale-95 translate-y-4 h-0 pointer-events-none'}
`}
 >
 {/* Header */}
 <div className="flex items-center justify-between px-4 py-3 bg-gradient-to-r from-indigo-500 to-purple-600 rounded-t-2xl text-white shrink-0">
 ...
 <span className="w-2 h-2 rounded-md bg-primary-400 animate-pulse"></span>
 <span className="text-[11px] text-indigo-100">オンライン</span>
 </div>
 </div>
 </div>
 ...
 >
 {msg.role === 'ai' && (
 <div className="w-7 h-7 rounded-md bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center shrink-0 mt-0.5">
 <Sparkles size={14} className="text-white"/>
 </div>
 )}
 {msg.role === 'user' && (
 <div className="w-7 h-7 rounded-md bg-gray-200 flex items-center justify-center shrink-0 mt-0.5">
 ...
 : 'bg-white border border-gray-350 text-[#1f1f1f] rounded-md rounded-tl-sm'
 }
`}
 >
 {msg.content}
 ...
 {/* Footer - Input Area */}
 <div className="p-3 bg-white border-t border-gray-350 rounded-b-2xl shrink-0">
 ...
 className="flex-1 bg-gray-50 border border-gray-350 focus:border-indigo-400 focus:ring-1 focus:ring-indigo-400 rounded-md pl-4 pr-11 py-2.5 text-[13px] text-[#1f1f1f] outline-none transition-all disabled:opacity-50"
 />
 <button
 type="submit"
 disabled={!inputValue.trim() || isTyping}
 className={`absolute right-1.5 p-1.5 rounded-md transition-all duration-200
...
`}
 ...
 onClick={toggleChat}
 className={`
 w-14 h-14 rounded-md flex items-center justify-center -500/30 transition-all duration-300 hover:scale-105 active:scale-95 group relative
...
`}
 ...
 <span className="relative inline-flex rounded-md h-3.5 w-3.5 bg-red-500 border-2 border-white"></span>
 </span>
 </>
 )}
 </button>
