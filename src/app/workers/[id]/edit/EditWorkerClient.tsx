...
export default function EditWorkerClient({ companies, worker }: { companies: any[], worker: any }) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            formPayload.append('id', worker.id);
            ...
            if (result.success) {
                ...
                router.refresh(); // Refresh page to reflect new changes
            } else {
        ...
        <div className="flex-1 flex flex-col h-full bg-white relative">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
                <div className="flex items-center gap-4">
                    <Link href={`/workers/${worker.id}`} className="w-10 h-10 flex items-center justify-center rounded-none hover:bg-gray-100 transition-colors text-[#1f1f1f]">
                    ...
                    <h2 className="text-[24px] font-medium tracking-tight text-[#1f1f1f] flex items-center gap-2">
                        ...
                        <span className="text-xs bg-[#24b47e]/10 text-[#24b47e] px-2 py-1 rounded-none border border-[#24b47e]/20 flex items-center gap-1 font-bold ml-2">
                            <Sparkles size={12} /> AI Workspace
                        </span>
                    </h2>
                </div>
                <div className="flex items-center gap-3">
                    <Link href={`/workers/${worker.id}`} className="px-5 py-2.5 text-[#1f1f1f] text-sm bg-white font-medium hover:bg-gray-50 rounded-md transition-colors border border-gray-300">
                        キャンセル (Cancel)
                    </Link>
                    <button
                        type="button"
                        ...
                        <div className="bg-white rounded-none border border-[#c4c8cf] overflow-hidden mb-6">
                            ...
                            <div className="flex flex-col">
                                <FormRow label="備考" isLast>
                                    <textarea name="remarks" value={formData.remarks} onChange={handleInputChange} rows={5} placeholder="実習生に関する特記事項やメモを自由に入力してください..." className="w-full bg-transparent focus:bg-gray-50/80 border-none px-3 py-2 text-sm outline-none text-[#1f1f1f] transition-all duration-300 ring-0 focus:ring-0 resize-y min-h-[100px]" />
                                </FormRow>
                            </div>
                        </div>
