...
export function AutoDocButton({ worker, role = 'admin' }: { worker: any, role?: string }) {
 ...
 <button onClick={() => setIsOpen(!isOpen)} disabled={isExporting} className="flex items-center gap-2 px-5 py-2.5 bg-[#311b92] text-white hover:bg-[#4527a0] rounded-md text-sm font-bold transition-colors disabled:opacity-50">
 ...
 {isOpen && (
 <div className="absolute top-full mt-2 right-0 w-64 bg-white rounded-md border border-gray-350 py-2 z-50 overflow-hidden">
 ...
 <button onClick={() => generateDoc('joken')} className="w-full text-left px-4 py-3 text-sm font-bold text-[#1f1f1f] hover:bg-gray-50 transition-colors border-t border-gray-350">📄 雇用条件書 (自動作成)</button>
 </div>
 )}
 </div>
 )
