...
/**
 * Automates the creation of 1st, 3rd, and 6th-month regular audits * for newly registered Technical Interns (Ginou Jisshu) or Specified Skilled Workers (Ikusei Shuro).
 */
export async function autoScheduleAuditsForWorkers(workers: Partial<Worker>[]) {
 ...
 // Optional: Only apply to specific system types if required by law
 // if (worker.system_type !== 'ginou_jisshu' && worker.system_type !== 'ikusei_shuro') return;
 ...
 audit_type: 'kansa', // 定期監査 (Regular Audit)
 status: 'planned',
 scheduled_date: scheduledDate.toISOString().split('T')[0],
 notes:`システム自動生成: ${worker.full_name_romaji} 様の入国/配属 ${months}ヶ月後定期監査`
 });
 });
 });
...
export async function syncWorkerStatusFromProcedure(worker_id: string, procedure_name: string, status: string) {
 ...
 if (name.includes('認定') || name.includes('COE') || name.includes('coe')) {
