                    ...
                    <form action={createAudit} className="space-y-6">
                        <div className="bg-white rounded-[32px] shadow-sm border border-[#e1e5ea] p-8">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="md:col-span-2"><label className="block text-sm font-medium text-[#444746] mb-2">対象企業 <span className="text-red-500">*</span></label><select name="company_id" required className="w-full bg-[#f0f4f9] focus:bg-white border border-transparent focus:border-[#4285F4] rounded-2xl px-4 py-3 outline-none appearance-none cursor-pointer text-[#1f1f1f] transition-all"><option value="">選択してください</option>{companies?.map(c => <option key={c.id} value={c.id}>{c.name_jp}</option>)}</select></div>
                                <div><label className="block text-sm font-medium text-[#444746] mb-2">業務区分 <span className="text-red-500">*</span></label><select name="audit_type" required defaultValue="homon" className="w-full bg-[#f0f4f9] focus:bg-white border border-transparent focus:border-[#4285F4] rounded-2xl px-4 py-3 outline-none appearance-none cursor-pointer text-[#1f1f1f] transition-all"><option value="homon">定期訪問 (毎月)</option><option value="kansa">定期監査 (3ヶ月に1回)</option><option value="rinji">臨時対応・その他</option></select></div>
                                ...
                                <div><label className="block text-sm font-medium text-[#444746] mb-2">担当スタッフ</label><input name="pic_name" type="text" defaultValue={userProfile?.full_name || ''} className="w-full bg-[#f0f4f9] focus:bg-white border border-transparent focus:border-[#4285F4] rounded-2xl px-4 py-3 outline-none text-[#1f1f1f] transition-all" /></div>
                                <div><label className="block text-sm font-medium text-[#444746] mb-2">初期ステータス</label><select name="status" defaultValue="planned" className="w-full bg-[#f0f4f9] focus:bg-white border border-transparent focus:border-[#4285F4] rounded-2xl px-4 py-3 outline-none appearance-none cursor-pointer text-[#1f1f1f] transition-all"><option value="planned">予定（未実施）</option><option value="in_progress">報告書作成中</option><option value="completed">完了（提出済）</option></select></div>
                        ...
                        <div className="pt-4 pb-8 flex justify-end gap-3 sticky bottom-4"><Link href="/audits" className="px-6 py-3 text-[#444746] bg-white border border-[#e1e5ea] font-medium hover:bg-gray-50 rounded-full transition-colors shadow-sm">キャンセル</Link><SaveButton /></div>
                    </form>
                </div>
            </main>
        </div>
