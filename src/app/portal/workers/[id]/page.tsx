    ...
    return (
        <div className="max-w-[1000px] mx-auto">
            <div className="flex items-center gap-4 mb-8">
                <Link href="/portal/workers" className="w-10 h-10 flex items-center justify-center rounded-md bg-white hover:bg-gray-50 text-gray-900"><ArrowLeft size={20} /></Link>
                <h2 className="text-[28px] font-bold text-primary-800">人材プロフィール・書類管理</h2>
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-1">
                    <div className="bg-white rounded-md p-6 flex flex-col items-center text-center">
                        <div className="w-24 h-24 rounded-md bg-white flex items-center justify-center border border-gray-350 overflow-hidden mb-4">
                            {worker.avatar_url ? <img src={worker.avatar_url} className="w-full h-full object-cover" /> : <UserCircle2 size={40} className="text-primary-400" />}
                        </div>
                        ...
                        <div className="w-full mt-6 space-y-3 text-left bg-white p-4 rounded-md">
                            <div><p className="text-[10px] text-gray-500 font-bold">在留カード番号</p><p className="text-sm font-mono font-bold text-gray-900">{worker.zairyu_no || '-'}</p></div>
                            <div><p className="text-[10px] text-gray-500 font-bold">パスポート期限</p><p className="text-sm font-bold text-red-600">{worker.passport_exp?.replace(/-/g, '/') || '-'}</p></div>
                        </div>
                    </div>
                </div>

                <div className="lg:col-span-2">
                    <div className="mb-4 bg-white border border-gray-350 rounded-md p-4 text-sm text-primary-800 font-medium">
