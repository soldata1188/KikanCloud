...
export default async function AccountsPage() {
    ...
    const getRoleBadge = (role: string) => {
        switch (role) {
            ...
            case 'staff': return <span className="bg-white text-[#1f1f1f] px-2 py-0.5 rounded-[4px] text-[10px] font-bold border border-gray-200 flex items-center gap-1 w-fit"><UserCircle2 size={10} /> 団体職員</span>
            case 'company_admin': return <span className="bg-white text-emerald-700 px-2 py-0.5 rounded-[4px] text-[10px] font-bold border border-emerald-200 flex items-center gap-1 w-fit"><Building2 size={10} /> 企業管理者</span>
            case 'company_user': return <span className="bg-white text-[#878787] px-2 py-0.5 rounded-[4px] text-[10px] font-bold border border-gray-200 flex items-center gap-1 w-fit"><UserCircle2 size={10} /> 企業ユーザー</span>
            case 'worker': return <span className="bg-white text-[#878787] px-2 py-0.5 rounded-[4px] text-[10px] font-bold border border-gray-200 flex items-center gap-1 w-fit">実習生・労働者</span>
            default: return null
        }
    }
                ...
                <main className="flex-1 overflow-y-auto p-6 md:p-10">
                    <div className="max-w-[1200px] mx-auto">
                        <div className="flex items-center gap-4 mb-8">
                            <Link href="/" className="w-8 h-8 flex items-center justify-center rounded-md border border-gray-200 hover:bg-[#ededed] text-[#878787] hover:text-[#1f1f1f] transition-colors"><ArrowLeft size={16} /></Link>
                            <div>
                            ...
                            <div className="lg:col-span-1">
                                <div className="bg-white rounded-lg p-6 shadow-sm border border-gray-200 sticky top-8">
                                    ...
                                    <form action={createAccount} className="space-y-4">
                                        <div><label className="block text-[11px] font-medium text-[#878787] mb-1.5 uppercase tracking-wider">氏名</label><input type="text" name="fullName" required className="w-full h-[36px] bg-white border border-gray-200 focus:bg-white focus:border-[#878787] rounded-md px-3 text-[13px] text-[#1f1f1f] outline-none transition-colors" /></div>
                                        <div><label className="block text-[11px] font-medium text-[#878787] mb-1.5 uppercase tracking-wider">ログイン ID</label><input type="email" name="email" required className="w-full h-[36px] bg-white border border-gray-200 focus:bg-white focus:border-[#878787] rounded-md px-3 text-[13px] text-[#1f1f1f] outline-none transition-colors" /></div>
                                        <div><label className="block text-[11px] font-medium text-[#878787] mb-1.5 uppercase tracking-wider">パスワード</label><input type="password" name="password" required minLength={6} className="w-full h-[36px] bg-white border border-gray-200 focus:bg-white focus:border-[#878787] rounded-md px-3 text-[13px] text-[#1f1f1f] outline-none transition-colors" /></div>
                                        <div>
                                            <label className="block text-[11px] font-medium text-[#878787] mb-1.5 uppercase tracking-wider">権限ロール</label>
                                            <select name="role" required className="w-full h-[36px] bg-white border border-gray-200 focus:bg-white focus:border-[#878787] rounded-md px-3 text-[13px] text-[#1f1f1f] outline-none">
                                        ...
                                        <div>
                                            ...
                                            <select name="companyId" className="w-full h-[36px] bg-white border border-gray-200 focus:bg-white focus:border-[#878787] rounded-md px-3 text-[13px] text-[#878787] outline-none">
                            ...
                            <div className="lg:col-span-2">
                                <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                                    <div className="px-5 py-4 border-b border-gray-200 bg-white flex items-center justify-between">
                                        ...
                                        <span className="text-[10px] font-mono text-[#878787] bg-white px-2 py-0.5 rounded border border-gray-200">{users?.length || 0} 名</span>
                                    </div>
                                    <div className="overflow-x-auto">
                                        <table className="w-full text-left text-[13px] whitespace-nowrap">
                                            <thead className="bg-white text-[11px] font-medium text-[#878787] uppercase tracking-wider border-b border-gray-200">
                                                <tr>
