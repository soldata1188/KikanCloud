    ...
    return (
        <>
            <div onClick={() => setIsOpen(true)} className="relative hidden md:block group cursor-pointer">
                ...
                <div className="h-[30px] w-52 bg-[#fbfcfd] border border-[#ededed] rounded-md pl-8 pr-12 text-xs flex items-center text-[#a0a0a0] group-hover:border-[#24b47e] transition-all">Search...</div>
                <div className="absolute right-1.5 top-1/2 -translate-y-1/2 flex items-center gap-0.5 text-[#878787] text-[10px] font-mono">
                    <span className="border border-[#ededed] bg-white rounded px-1 shadow-sm group-hover:border-[#24b47e]/50">⌘</span><span className="border border-[#ededed] bg-white rounded px-1 shadow-sm group-hover:border-[#24b47e]/50">K</span>
                </div>
            </div>
            ...
            {isOpen && (
                <div className="fixed inset-0 z-[100] flex items-start justify-center pt-[15vh] px-4 bg-[#1f1f1f]/40 backdrop-blur-sm transition-opacity" onClick={() => setIsOpen(false)}>
                    <div className="w-full max-w-[600px] bg-white rounded-xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
                        <div className="flex items-center px-4 border-b border-[#ededed] relative">
                            <Search size={18} className="text-[#878787] shrink-0" />
                            <input ref={inputRef} value={query} onChange={e => setQuery(e.target.value)} placeholder="Tên TTS, Mã thẻ, hoặc Xí nghiệp..." className="w-full h-14 bg-transparent border-none outline-none px-3 text-[14px] text-[#1f1f1f] placeholder:text-[#a0a0a0] font-sans" />
                        ...
                        <div className="max-h-[60vh] overflow-y-auto p-2 bg-[#fbfcfd]">
                            {query.length < 2 && <div className="px-4 py-8 text-center text-[#878787] text-[13px]">Nhập ít nhất 2 ký tự để tìm kiếm toàn cầu...</div>}
                            {query.length >= 2 && results.workers.length === 0 && results.companies.length === 0 && !isPending && <div className="px-4 py-8 text-center text-[#878787] text-[13px]">Không tìm thấy kết quả cho "{query}"</div>}
                                    ...
                                    {results.workers.length > 0 && (
                                        <div>
                                            ...
                                            {results.workers.map(w => (
                                                <Link key={w.id} href={`/workers/${w.id}`} onClick={() => setIsOpen(false)} className="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-white hover:shadow-sm border border-transparent hover:border-[#ededed] group transition-all">
                                    ...
                                    {results.companies.length > 0 && (
                                        <div>
                                            ...
                                            {results.companies.map(c => (
                                                <Link key={c.id} href={`/companies/${c.id}`} onClick={() => setIsOpen(false)} className="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-white hover:shadow-sm border border-transparent hover:border-[#ededed] group transition-all">
                        ...
                        <div className="bg-white border-t border-[#ededed] px-4 py-2 flex items-center justify-between text-[10px] text-[#878787] font-medium">
                            <span><kbd className="bg-white border border-[#ededed] px-1.5 py-0.5 rounded shadow-sm mr-1">ESC</kbd> to close</span>
                        </div>
                    </div>
                </div>
            )}
