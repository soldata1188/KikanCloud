...
export function ImportModal() {
        ...
        Papa.parse(file, {
            ...
            complete: (results) => {
                ...
                const mappedData = validData.map((row: any) => ({
                    name_jp: row['企業名（日本語）'],
                    name_romaji: row['企業名（ローマ字/カナ）'],
                    corporate_number: row['法人番号'],
                    labor_insurance_number: row['労働保険番号'],
                    employment_insurance_number: row['雇用保険番号'],
                    acceptance_notification_number: row['受入通知番号'],
                    acceptance_notification_date: row['受入通知日'],
                    industry: row['業種'],
                    accepted_occupations: row['受入職種'],
                    postal_code: row['郵便番号'],
                    address: row['所在地（住所）'],
                    phone: row['電話番号'],
                    employee_count: row['従業員数'],
                    representative: row['代表者名'],
                    representative_romaji: row['代表者名（ふりがな）'],
                    manager_name: row['責任者'],
                    training_date: row['講習受講日'],
                    guidance_manager: row['指導責任者'],
    ...
    const downloadTemplate = () => {
        const headers = ['企業名（日本語）', '企業名（ローマ字/カナ）', '法人番号', '労働保険番号', '雇用保険番号', '受入通知番号', '受入通知日', '業種', '受入職種', '郵便番号', '所在地（住所）', '電話番号', '従業員数', '代表者名', '代表者名（ふりがな）', '責任者', '講習受講日', '指導責任者', '生活指導員', '技能指導員']
        const csvContent = "data:text/csv;charset=utf-8,\uFEFF" + headers.join(',') + '\n株式会社サンプル,SAMPLE CO.,1234567890123,1111-222222-3,1111-222222-3,12345,2024/01/01,製造業,溶接,100-0001,東京都千代田区,03-1234-5678,50,山田 太郎,ヤマダ タロウ,田中 健太,2024/01/01,佐藤 花子,高橋 次郎,伊藤 三郎'
