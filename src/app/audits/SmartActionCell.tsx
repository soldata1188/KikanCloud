...
export function SmartActionCell({ auditId, status, companyId, filterMonth }: { auditId: string | null, status: string | null, companyId: string, filterMonth: string }) {
 ...
 if (!auditId) {
 return (
 <Link href={`/audits/new?company_id=${companyId}&month=${filterMonth}`} className="inline-flex items-center justify-center gap-1.5 px-3 py-1.5 bg-white hover:bg-[#ededed] text-[#878787] hover:text-[#1f1f1f] text-[11px] font-medium rounded-lg transition-colors border border-gray-350 w-full">
 ...
 return (
 <div className="text-center w-full px-3 py-1.5 bg-white text-[#878787] text-[11px] font-medium rounded-lg border border-gray-350 flex items-center justify-center gap-1 opacity-80">
