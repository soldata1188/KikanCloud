...
export default function StaffList({ initialStaff, organizationId }: { initialStaff: any[], organizationId: string }) {
    ...
    const getRoleBadge = (role: string) => {
        switch (role) {
            ...
            case 'union_staff': return <span className="bg-[#fbfcfd] text-[#1f1f1f] px-2 py-0.5 rounded-[4px] text-[10px] font-bold border border-[#ededed] flex items-center gap-1 w-fit"><UserCircle2 size={10} /> 担当職員 (Staff)</span>
            default: return <span className="bg-[#fbfcfd] text-[#878787] px-2 py-0.5 rounded-[4px] text-[10px] border border-[#ededed] w-fit">{role}</span>
        }
    }
    ...
    return (
        <div>
            ...
            {isAddMode && (
                <div className="bg-[#fbfcfd] border border-[#ededed] rounded-lg p-6 mb-8">
                    ...
                    <form action="/api/organization/staff/create" method="POST" className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input name="fullName" required className="w-full text-sm border border-[#ededed] rounded px-3 py-2 outline-none focus:border-[#878787]" />
                        </div>
                        <div>
                            <label className="block text-xs font-medium text-[#878787] mb-1">ログイン Email</label>
                            <input name="email" type="email" required className="w-full text-sm border border-[#ededed] rounded px-3 py-2 outline-none focus:border-[#878787]" />
                        </div>
                        <div>
                            <label className="block text-xs font-medium text-[#878787] mb-1">仮パスワード (Min 6 chars)</label>
                            <input name="password" type="password" required minLength={6} className="w-full text-sm border border-[#ededed] rounded px-3 py-2 outline-none focus:border-[#878787]" />
                        </div>
                        <div>
                            <label className="block text-xs font-medium text-[#878787] mb-1">権限 (Role)</label>
                            <select name="role" className="w-full text-sm border border-[#ededed] rounded px-3 py-2 outline-none focus:border-[#878787]">
            ...
            <div className="border border-[#ededed] rounded-lg overflow-hidden">
                <table className="w-full text-left text-sm">
                    <thead className="bg-[#fbfcfd] text-xs font-medium text-[#878787] uppercase tracking-wider border-b border-[#ededed]">
                        <tr>
                    ...
                    <tbody className="divide-y divide-[#ededed] bg-white">
                        {initialStaff.map(s => (
                            <tr key={s.id} className="hover:bg-[#fbfcfd]">
