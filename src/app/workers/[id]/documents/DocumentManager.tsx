...
export function DocumentManager({ workerId, documents, role }: { workerId: string, documents: any[], role: string }) {
    ...
    const getTypeStyle = (type: string) => {
        switch (type) {
            ...
            default: return { label: 'その他資料', color: 'bg-gray-100 text-gray-700', icon: <FileText size={24} className="text-[#878787]" /> }
        }
    }
 ...
 <div className="bg-white rounded-md p-6 sticky top-24"> 
                    <div className="mb-6">
                        <h3 className="text-lg font-bold text-[#1f1f1f] flex items-center gap-2"><Upload size={20} className="text-[#24b47e]" /> 新規アップロード</h3>
                        <p className="text-xs text-[#878787] mt-1">PDF, JPG, PNG形式対応（最大5MB）</p>
                    </div>
 ...
 <select value={docType} onChange={(e) => setDocType(e.target.value)} disabled={uploading || isPending} className="w-full bg-white -transparent focus: focus:bg-white rounded-md px-4 py-3 text-sm font-medium outline-none transition-colors cursor-pointer"> 
                        ...
                        >
                            ...
                            <div className="flex flex-col items-center justify-center gap-2">
                                {uploading ? <Loader2 size={32} className="text-[#24b47e] animate-spin mb-1" /> : <Upload size={32} className="text-[#878787] group-hover:text-[#24b47e] transition-colors mb-1" />}
                                <p className="text-[#1f1f1f] font-medium text-sm">{uploading ? 'アップロード中...' : 'クリックしてファイルを選択'}</p>
                                <p className="text-[10px] text-[#878787]">またはドラッグ＆ドロップ</p>
                            </div>
                        </div>
                    </div>
                </div>
 ...
 <span className="text-xs font-bold text-[#878787] bg-gray-200 px-3 py-1 rounded-md flex items-center gap-1"><ShieldCheck size={14} /> {documents.length}件</span>
                    ...
                    {documents.length === 0 ? (
                        <div className="p-16 text-center flex flex-col items-center gap-4 text-[#878787]">
                                    ...
                                    <li key={doc.id} className="p-4 hover:bg-gray-50/30 transition-colors flex items-center justify-between gap-4 group">
                                        <div className="flex items-center gap-4 min-w-0 flex-1">
                                            ...
                                            <div className="min-w-0 flex-1">
                                                ...
                                                <p className="text-[11px] text-[#878787] flex items-center gap-2">
                                                    <span>{formatBytes(doc.file_size)}</span>
 <span className="w-1 h-1 bg-gray-300 rounded-md"></span>
 ...
 <button type="submit" onClick={(e) => { if (!confirm('このファイルを完全に削除しますか？')) e.preventDefault() }} disabled={isPending} className="p-2.5 text-[#878787] hover:text-red-600 hover:bg-red-50 rounded-md transition-colors disabled:opacity-50" title="削除">
                                                        <Trash2 size={18} />
                                                    </button>
                                                </form>
                                            )}
