        ...
        <div className="bg-white border border-gray-200 rounded-md shadow-sm flex flex-col md:flex-row hover:border-blue-300 transition-colors pointer-events-none md:pointer-events-auto">
            ...
            <div className="w-full md:w-[380px] shrink-0 border-b md:border-b-0 md:border-r border-gray-200 p-4 flex flex-col md:flex-row gap-3 relative">
                ...
                <div className="absolute top-4 right-4 flex gap-1">
                    <span className="px-1.5 py-0.5 bg-green-100 text-green-700 text-[10px] font-bold rounded-sm border border-green-200">在籍中</span>
                    {isWarnExp && (
                        <span className="px-1.5 py-0.5 bg-red-100 text-red-700 text-[10px] font-bold rounded-sm border border-red-200">
                            期限まで{daysUntilExp}日
                        </span>
                    )}
                </div>
                ...
                <div className="flex-1 mt-6 md:mt-0">
                    <div className="flex items-center gap-3 mb-3">
                        <div className="w-10 h-10 rounded-full border border-gray-200 bg-slate-100 flex items-center justify-center text-slate-400 font-bold">
                            {worker.full_name_romaji.charAt(0)}
                        </div>
                        <div>
                            <div className="text-[10px] text-gray-500 font-semibold">{worker.full_name_kana}</div>
                            <div className="text-sm font-bold text-gray-900 leading-tight">{worker.full_name_romaji}</div>
                            <div className="text-[10px] text-[#0067b8] font-semibold mt-0.5">{worker.company_name}</div>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-y-2 text-[11px]">
                        <div><span className="text-gray-400">資格:</span> <span className="font-bold text-[#0067b8]">{worker.visa_status}</span></div>
                        <div><span className="text-gray-400">期限:</span> <span className={`font-bold ${isWarnExp ? 'text-red-600' : 'text-gray-700'}`}>{worker.zairyu_exp}</span></div>
                        <div><span className="text-gray-400">入国:</span> <span className="font-medium text-gray-700">{worker.entry_date}</span></div>
                        <div><span className="text-gray-400">経過:</span> <span className="font-bold text-gray-900">{years}年 {months}ヶ月</span></div>
            ...
            <div className="w-full md:w-[260px] shrink-0 border-b md:border-b-0 md:border-r border-gray-200 p-4 relative md:group md:cursor-pointer md:hover:bg-gray-50 transition-colors">
                ...
                <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-xs font-bold text-gray-900">機構業務</span>
                    <span className={`px-1.5 py-0.5 text-[10px] font-bold rounded border ${worker.kikou_status?.progress === '完了'
                ...
                <div className="space-y-1.5 text-[11px]">
                    <div className="flex justify-between"><span className="text-gray-400">申請内容:</span> <span className="font-bold text-gray-800">{worker.kikou_status?.type || '---'}</span></div>
                    <div className="flex justify-between"><span className="text-gray-400">申請日:</span> <span className="font-medium text-gray-700">{worker.kikou_status?.application_date || '---'}</span></div>
                    <div className="flex justify-between"><span className="text-gray-400">担当者:</span> <span className="font-medium text-gray-700">{worker.kikou_status?.assignee || '---'}</span></div>
                </div>
            </div>
            ...
            <div className="w-full md:w-[260px] shrink-0 border-b md:border-b-0 md:border-r border-gray-200 p-4 relative md:group md:cursor-pointer md:hover:bg-gray-50 transition-colors">
                ...
                <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-xs font-bold text-gray-900">入管業務</span>
                    <span className={`px-1.5 py-0.5 text-[10px] font-bold rounded border ${worker.nyukan_status?.progress === '審査中'
                ...
                <div className="space-y-1.5 text-[11px]">
                    <div className="flex justify-between"><span className="text-gray-400">申請日:</span> <span className="font-bold text-[#0067b8]">{worker.nyukan_status?.application_date || '---'}</span></div>
                    <div className="flex justify-between"><span className="text-gray-400">受理番号:</span> <span className="font-medium text-gray-700">{worker.nyukan_status?.receipt_number || '---'}</span></div>
                </div>
            </div>
            ...
            <div className="flex-1 p-4 bg-gray-50/30">
                <div className="flex justify-between items-center mb-2">
                    <span className="text-[11px] font-bold text-gray-500">メモ (Memo)</span>
