...
export function SmartDrawer({ workerId, documents, title, docType, icon, allowUpload = true }: { workerId: string, documents: any[], title: string, docType: string, icon: React.ReactNode, allowUpload?: boolean }) {
    ...
    return (
        <div className="bg-white rounded-[24px] shadow-sm border border-teal-100 overflow-hidden mb-4">
            <button onClick={() => setIsOpen(!isOpen)} className="w-full px-4 py-2 flex items-center justify-between bg-white hover:bg-teal-50/50 transition-colors">
                <div className="flex items-center gap-3">
                    <div className={`w-10 h-10 rounded-[32px] flex items-center justify-center shrink-0 ${allowUpload ? 'bg-orange-50 text-orange-600' : 'bg-teal-50 text-teal-600'}`}>{icon}</div>
