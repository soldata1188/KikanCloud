...
export function BulkInputClient({ companies }: { companies: { id: string, name_jp: string }[] }) {
    ...
    return (
        <div className="flex-1 flex flex-col items-start w-full relative">
            <div className="w-full overflow-x-auto border border-[#e1e5ea] rounded-xl mb-4">
                        ...
                        {rows.map((row, idx) => (
                            <tr key={row.id} className="hover:bg-gray-50/50 group">
                                ...
                                <td className="px-1 py-1">
                                    <input type="text" value={row.full_name_romaji} onChange={(e) => handleChange(row.id, 'full_name_romaji', e.target.value)} placeholder="NGUYEN VAN A" className="w-full px-3 py-2 uppercase outline-none border border-transparent focus:border-[#4285F4] focus:bg-white rounded transition-colors bg-transparent hover:bg-white/50" />
                                </td>
                                <td className="px-1 py-1">
                                    <input type="text" value={row.full_name_kana} onChange={(e) => handleChange(row.id, 'full_name_kana', e.target.value)} placeholder="グエン ヴァン ア" className="w-full px-3 py-2 outline-none border border-transparent focus:border-[#4285F4] focus:bg-white rounded transition-colors bg-transparent hover:bg-white/50" />
                                </td>
                                <td className="px-1 py-1">
                                    <input type="date" value={row.dob} onChange={(e) => handleChange(row.id, 'dob', e.target.value)} className="w-full px-3 py-2 outline-none border border-transparent focus:border-[#4285F4] focus:bg-white rounded transition-colors bg-transparent hover:bg-white/50" />
                                </td>
                                <td className="px-1 py-1">
                                    <select value={row.system_type} onChange={(e) => handleChange(row.id, 'system_type', e.target.value)} className="w-full px-3 py-2 outline-none border border-transparent focus:border-[#4285F4] focus:bg-white rounded transition-colors bg-white hover:bg-gray-50">
                                ...
                                <td className="px-1 py-1">
                                    <select value={row.status} onChange={(e) => handleChange(row.id, 'status', e.target.value)} className="w-full px-3 py-2 outline-none border border-transparent focus:border-[#4285F4] focus:bg-white rounded transition-colors bg-white hover:bg-gray-50">
                                ...
                                <td className="px-1 py-1">
                                    <select value={row.company_id} onChange={(e) => handleChange(row.id, 'company_id', e.target.value)} className="w-full px-3 py-2 outline-none border border-transparent focus:border-[#4285F4] focus:bg-white rounded transition-colors bg-white hover:bg-gray-50">
                                        <option value="">-- 未配属 --</option>
                                        {companies.map(c => <option key={c.id} value={c.id}>{c.name_jp}</option>)}
                                    </select>
                                </td>
                                <td className="px-1 py-1">
                                    <input type="text" value={row.zairyu_no} onChange={(e) => handleChange(row.id, 'zairyu_no', e.target.value)} placeholder="XX123456" className="w-full px-3 py-2 uppercase font-mono outline-none border border-transparent focus:border-[#4285F4] focus:bg-white rounded transition-colors bg-transparent hover:bg-white/50" />
                                </td>
                                <td className="px-3 py-2 text-center">
                                    <button onClick={() => removeRow(row.id)} disabled={rows.length <= 1} className="p-1.5 text-gray-400 hover:text-red-500 rounded hover:bg-red-50 transition-colors disabled:opacity-30">
                                        <Trash2 size={16} />
                                    </button>
                                </td>
                            </tr>
