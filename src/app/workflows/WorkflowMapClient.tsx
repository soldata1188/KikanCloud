...
const CustomNode = ({ data, selected, isConnectable }: { data: any, selected: boolean, isConnectable: boolean }) => (
 <div className={`bg-white border ${selected ? 'border-[#24b47e] ring-1 ring-[#24b47e]/20' : 'border-gray-350 hover:border-[#878787]'} rounded-md w-[280px] p-4 transition-all duration-200 cursor-pointer`}>
 ...
 <div className={`w-6 h-6 rounded flex items-center justify-center text-[11px] font-bold ${data.status === 'done' ? 'bg-[#24b47e] text-white' : 'bg-white border border-gray-350 text-[#878787]'}`}>{data.step || '?'}</div>
...
export default function WorkflowMapClient({ dbNodes, dbEdges, role }: { dbNodes: any[], dbEdges: any[], role: string }) {
 ...
 return (
 <div className="w-full h-full relative bg-white border border-gray-350 rounded-md overflow-hidden flex">
 {/* MAP AREA */}
 <div className="flex-1 h-full relative">
 <div className="absolute top-4 left-4 z-10 flex flex-col gap-2">
 <div className="bg-white/90 backdrop-blur-sm border border-gray-350 px-4 py-2.5 rounded-md">
 ...
 {canEdit && (
 <div className="flex items-center gap-2 bg-white/90 backdrop-blur-sm border border-gray-350 p-1.5 rounded-md w-fit">
 {!isEditing ? (
 <button onClick={() => { setIsEditing(true); setSelectedNodeId(null); }} className="flex items-center gap-1.5 px-3 py-1.5 text-[11px] font-bold text-[#1f1f1f] bg-white border border-gray-350 rounded hover:bg-white transition-colors"><Edit2 size={12} /> 編集モード</button>
 ) : (
 <>
 ...
 <Controls className="!bg-white !border !border-gray-350 !rounded-md ! fill-[#878787] mb-4 ml-4"showInteractive={false} />
 <MiniMap className="!bg-white !border !border-gray-350 !rounded-md ! mb-4 mr-4 hidden md:block"maskColor="rgba(255, 255, 255, 0.7)"nodeColor="#24b47e"/>
 </ReactFlow>
 </div>

 {/* SIDE PANEL (SOP / EDIT FORM) */}
 <div className={`absolute top-0 right-0 h-full w-full sm:w-[420px] bg-white border-l border-gray-350 transition-transform duration-300 ease-[cubic-bezier(0.34,1.56,0.64,1)] z-20 flex flex-col ${selectedNode ? 'translate-x-0' : 'translate-x-full'}`}>
 {selectedNode && (
 <div className="w-full h-full flex flex-col">
 <div className="p-6 border-b border-gray-350 flex items-start justify-between bg-white shrink-0">
 <div className="flex-1 pr-4">
 {isEditing ? (
 <div className="space-y-3 w-full">
 <div className="flex gap-2">
 <input type="text"value={selectedNode.data.step || ''} onChange={e => updateNodeData('step', e.target.value)} placeholder="番号"className="w-16 px-2.5 py-1.5 bg-white border border-gray-350 rounded-lg text-xs outline-none focus:border-[#24b47e]"/>
 <input type="text"value={selectedNode.data.phase || ''} onChange={e => updateNodeData('phase', e.target.value)} placeholder="区分"className="flex-1 px-2.5 py-1.5 bg-white border border-gray-350 rounded-lg text-xs outline-none focus:border-[#24b47e] tracking-wider"/>
 </div>
 <input type="text"value={selectedNode.data.label || ''} onChange={e => updateNodeData('label', e.target.value)} placeholder="タイトル"className="w-full px-2.5 py-2 bg-white border border-gray-350 rounded-lg text-lg font-bold text-[#1f1f1f] outline-none focus:border-[#24b47e]"/>
 </div>
 ) : (
 <>
 <span className="px-2.5 py-1 border border-gray-350 text-[#878787] rounded-[4px] text-[10px] tracking-wider bg-white mb-3 inline-block">手順 {selectedNode.data.step} : {selectedNode.data.phase}</span>
 <h2 className="text-xl font-bold text-[#1f1f1f] leading-tight">{selectedNode.data.label}</h2>
 </>
 )}
 </div>
 <button onClick={() => setSelectedNodeId(null)} className="w-8 h-8 flex items-center justify-center rounded-lg border border-gray-350 hover:bg-gray-50 hover:border-gray-350 text-[#878787] hover:text-[#1f1f1f] transition-all"><X size={18} /></button>
 </div>
 ...
 <select value={selectedNode.data.status || 'pending'} onChange={e => updateNodeData('status', e.target.value)} className="w-full px-3 py-2 bg-white border border-gray-350 rounded-lg text-xs outline-none focus:border-[#24b47e]">
 ...
 <input type="text"value={selectedNode.data.duration || ''} onChange={e => updateNodeData('duration', e.target.value)} className="w-full px-3 py-2 bg-white border border-gray-350 rounded-lg text-xs outline-none focus:border-[#24b47e]"/>
 </div>
 <div>
 <h3 className="text-[10px] font-bold text-[#878787] uppercase mb-1.5 flex items-center gap-1.5"><BookOpen size={12} /> 業務マニュアル</h3>
 <textarea rows={6} value={selectedNode.data.content || ''} onChange={e => updateNodeData('content', e.target.value)} className="w-full px-3 py-2 bg-white border border-gray-350 rounded-lg text-xs outline-none focus:border-[#24b47e] resize-none leading-relaxed"/>
 </div>
 <div>
 <h3 className="text-[10px] font-bold text-[#878787] uppercase mb-1.5 flex items-center gap-1.5"><FileText size={12} /> 必要書類（カンマ区切り）</h3>
 <textarea rows={3} value={(selectedNode.data.docs || []).join(', ')} onChange={e => updateNodeData('docs', e.target.value.split(',').map((s: string) => s.trim()).filter(Boolean))} placeholder="履歴書, パスポート, 申請書..."className="w-full px-3 py-2 bg-white border border-gray-350 rounded-lg text-xs outline-none focus:border-[#24b47e] resize-none leading-relaxed"/>
 </div>
 <div className="pt-4 border-t border-gray-350">
 <button onClick={handleDeleteNode} className="flex justify-center items-center gap-1.5 w-full py-2.5 text-xs font-bold text-red-600 bg-red-50 hover:bg-red-100 rounded-lg transition-colors"><Trash2 size={14} /> 削除</button>
 </div>
 </div>
 ) : (
 ...
 <div className="text-[13px] text-[#444746] leading-relaxed bg-white border border-gray-350 p-4 rounded-md whitespace-pre-wrap">{selectedNode.data.content}</div>
 </div>
 ...
 <li key={idx} className="flex items-center gap-2.5 text-[13px] font-medium text-[#1f1f1f] bg-white border border-gray-350 p-3 rounded-md">
 ...
 <div className="p-4 border-t border-gray-350 bg-white text-center shrink-0">
