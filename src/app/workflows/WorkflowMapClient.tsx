...
const CustomNode = ({ data, selected, isConnectable }: { data: any, selected: boolean, isConnectable: boolean }) => (
    <div className={`bg-white border ${selected ? 'border-[#24b47e] shadow-md ring-1 ring-[#24b47e]/20' : 'border-[#ededed] shadow-sm hover:border-[#878787]'} rounded-xl w-[280px] p-4 transition-all duration-200 cursor-pointer`}>
...
export default function WorkflowMapClient({ dbNodes, dbEdges, role }: { dbNodes: any[], dbEdges: any[], role: string }) {
    ...
    return (
        <div className="w-full h-full relative bg-white border border-[#ededed] rounded-lg shadow-sm overflow-hidden flex">
            {/* MAP AREA */}
            <div className="flex-1 h-full relative">
                <div className="absolute top-4 left-4 z-10 flex flex-col gap-2">
                    <div className="bg-white/90 backdrop-blur-sm border border-[#ededed] px-4 py-2.5 rounded-md shadow-sm">
                    ...
                    {canEdit && (
                        <div className="flex items-center gap-2 bg-white/90 backdrop-blur-sm border border-[#ededed] p-1.5 rounded-md shadow-sm w-fit">
                            {!isEditing ? (
                ...
                >
                    ...
                    <Controls className="!bg-white !border !border-[#ededed] !rounded-md !shadow-sm fill-[#878787] mb-4 ml-4" showInteractive={false} />
                    <MiniMap className="!bg-[#fbfcfd] !border !border-[#ededed] !rounded-md !shadow-sm mb-4 mr-4 hidden md:block" maskColor="rgba(255, 255, 255, 0.7)" nodeColor="#24b47e" />
                </ReactFlow>
            </div>

            {/* SIDE PANEL (SOP / EDIT FORM) */}
            <div className={`absolute top-0 right-0 h-full w-full sm:w-[420px] bg-white border-l border-[#ededed] shadow-2xl transition-transform duration-300 ease-[cubic-bezier(0.34,1.56,0.64,1)] z-20 flex flex-col ${selectedNode ? 'translate-x-0' : 'translate-x-full'}`}>
                {selectedNode && (
                    <div className="w-full h-full flex flex-col">
                        <div className="p-6 border-b border-[#ededed] flex items-start justify-between bg-white shrink-0">
                            ...
                            <button onClick={() => setSelectedNodeId(null)} className="w-8 h-8 flex items-center justify-center rounded-md border border-transparent hover:bg-[#fbfcfd] hover:border-[#ededed] text-[#878787] hover:text-[#1f1f1f] transition-all"><X size={18} /></button>
                        </div>

                        <div className="flex-1 overflow-y-auto p-6 bg-[#fbfcfd]">
                            {isEditing ? (
