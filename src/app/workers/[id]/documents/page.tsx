            ...
            <main className="flex-1 flex flex-col relative overflow-y-auto no-scrollbar">
                <div className="flex-1 flex flex-col px-4 pb-12 w-full max-w-[1100px] mx-auto mt-4 md:mt-8">
                    ...
                    <div className="flex flex-wrap items-center gap-2 mb-8 p-1.5 bg-white border border-[#ededed] rounded-[20px] w-fit shadow-sm">
                        <Link href={`/workers/${id}/edit`} className="flex items-center gap-2 px-6 py-2.5 rounded-[14px] text-sm font-bold transition-colors text-[#878787] hover:text-gray-800 hover:bg-[#fbfcfd]"><FileText size={18} /> 基本情報</Link>
                        <Link href={`/workers/${id}/documents`} className="flex items-center gap-2 px-6 py-2.5 rounded-[14px] text-sm font-bold transition-colors bg-[#fbfcfd] text-[#24b47e] shadow-sm pointer-events-none"><FolderOpen size={18} /> 関連書類</Link>
                        <Link href={`/workers/${id}/timeline`} className="flex items-center gap-2 px-6 py-2.5 rounded-[14px] text-sm font-bold transition-colors text-[#878787] hover:text-gray-800 hover:bg-[#fbfcfd]"><Map size={18} /> 手続ロードマップ</Link>
                    </div>
                    ...
                    <div className="mt-8 bg-white rounded-md shadow-sm border border-teal-100 overflow-hidden mb-8">
                        <div className="px-6 py-5 border-b border-[#ededed] flex items-center justify-between bg-[#fbfcfd]/30">
                        ...
                        {clientDocsUrls.length === 0 ? <div className="p-8 text-center text-sm text-[#878787]">提出された書類はありません。</div> : (
                            <ul className="divide-y divide-gray-100">
                                {clientDocsUrls.map(doc => (
                                    <li key={doc.id} className="p-4 hover:bg-[#fbfcfd]/30 transition-colors flex items-center justify-between gap-4">
                                        ...
                                        {doc.signedUrl && <a href={doc.signedUrl} target="_blank" rel="noopener noreferrer" className="p-2.5 bg-white border border-[#ededed] text-teal-600 hover:bg-[#fbfcfd] rounded-xl transition-colors font-bold text-xs flex items-center gap-1 shadow-sm shrink-0"><Download size={14} /> 開く</a>}
                                    </li>
                                ))}
                            </ul>
                        )}
