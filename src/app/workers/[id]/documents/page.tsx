            ...
            <main className="flex-1 flex flex-col relative overflow-y-auto no-scrollbar">
                <div className="flex-1 flex flex-col px-4 pb-12 w-full max-w-[1100px] mx-auto mt-4 md:mt-8">
                    ...
                    <div className="flex flex-wrap items-center gap-2 mb-8 p-1.5 bg-white border border-gray-300 rounded-[20px] w-fit">
                        ...
                        <Link href={`/workers/${id}/documents`} className="flex items-center gap-2 px-6 py-2.5 rounded-[14px] text-sm font-bold transition-colors bg-[#24b47e]/10 text-[#24b47e] pointer-events-none"><FolderOpen size={18} /> 関連書類</Link>
                    ...
                    <div className="mt-8 bg-white rounded-md border border-teal-200 overflow-hidden mb-8">
                            ...
                            <ul className="divide-y divide-gray-100">
                                {clientDocsUrls.map(doc => (
                                    <li key={doc.id} className="p-4 hover:bg-gray-50/30 transition-colors flex items-center justify-between gap-4">
                                        ...
                                        {doc.signedUrl && <a href={doc.signedUrl} target="_blank" rel="noopener noreferrer" className="p-2.5 bg-white border border-gray-300 text-teal-600 hover:border-teal-500 rounded-xl transition-colors font-bold text-xs flex items-center gap-1 shrink-0"><Download size={14} /> 開く</a>}
                                    </li>
                                ))}
                            </ul>
                        )}
