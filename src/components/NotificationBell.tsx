...
export function NotificationBell({ role, companyId }: { role: string, companyId?: string | null }) {
 ...
 {notifications.length > 0 && <span className="absolute -top-1 -right-1 w-5 h-5 bg-red-500 text-white text-[10px] font-bold rounded-md flex items-center justify-center border-2 border-white">{notifications.length}</span>}
 </button>

 {isOpen && (
 <div className="absolute top-full mt-2 right-0 w-80 bg-white rounded-lg border border-gray-350 overflow-hidden z-50 animate-in fade-in slide-in-from-top-2">
 <div className="px-4 py-3 bg-white border-b border-gray-350 flex justify-between items-center">
 ...
 <div className="p-6 text-center text-sm text-[#878787] flex flex-col items-center"><CheckCircle2 size={32} className="mb-2 text-primary-300 opacity-50"/>新しい通知はありません</div>
 ) : notifications.map(n => (
